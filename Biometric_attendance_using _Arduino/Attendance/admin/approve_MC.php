...
<body>
  ...
  {
    $sql1 = "select * from MC_request where course_id = '$course_id' ";
    $result1 = mysqli_query($con,$sql1);  
    $count1=mysqli_num_rows($result1);
    if($count1 > 0){
    echo "<h4><center>Medical Leave requests waiting for approval: </center></h4>";
      ...
      if($count2 > 0)
      {
        while ($row2 = mysqli_fetch_array($result2)) 
        {
          if(isset($_POST[$row2['username']]))
          {
            ...
            $sql3 = "delete from MC_request where username = '$rollno1' AND date = '$date1' AND course_id = '$course_id1' ";    
            $result3 =  mysqli_query($con,$sql3); 
          }
        }
      }

      $sql4 = "select * from MC_request where course_id = '$course_id' ";
      ...
      if($count2 > 0)
      {
        while ($row2 = mysqli_fetch_array($result2)) 
        {
          if(isset($_POST[$row2['username']]))
          {
            ...
            $date1 = $row2['date'];
            $course_id1 = $row2['course_id'];
            $sql3a = "delete from MC_request where username = '$rollno1' AND date = '$date1' AND course_id = '$course_id1' ";    
            $result3a =  mysqli_query($con,$sql3a); 
            $sql3b = "INSERT INTO `$course_id`(`username`, `date`, `present`, `MC_OD`) VALUES ('$rollno1', '$date1', '1', '1')";    
