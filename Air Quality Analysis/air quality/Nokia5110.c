...
// Measures the concentration of co2 and NH3 and displayed on the Nokia5110.
// Use SSI0 to send an 8-bit code to the Nokia5110 48x84
// pixel LCD to display text, images, or other information.

// N.Ramya - COE12B019, P.Madhuri - COE12B023
// November 10, 2015
...
float getResistance_NH3(int val) {
  //int val = analogRead(_pin);
  return ((1023./(float)val) * 5. - 1.)*RLOAD;
}


float getPPM_NH3(int val) {
  return 37.588 * pow((getResistance(val)/RZERO_NH3), -3.2353);
}


float getRZero_NH3(int val) {
  return getResistance(val) * pow((25/37.588), (1./3.2353));
}
...
int main(void){
 //unsigned char i;
  ...
  char string3[20]; // global to assist in debugging
	//char string1[20];
	char string1[20] = {'C','o','n','c','C','O','2','(','P','P','M',')'};
	char string2[20] = {' ',' ','C','o','n','c','N','H','3','(','P','P','M',')'};
	char s2[10];
	float op,op2;
